...
class DataPreparation:
    ...
    def stratified_splitter(self, df, labels):
        """
        Stratified split based on labels to ensure the class distribution is similar in both train and valid sets.
        """
        skf = StratifiedKFold(n_splits=int(1/self.valid_pct), shuffle=True, random_state=self.seed)
        for _, valid_idx in skf.split(df, labels):
            return valid_idx  # Return the validation indices (the rest will be for training)
    ...
    def load_data(self):
        ...
        try:
            # Stratified split
            valid_idx = self.stratified_splitter(df, df[df.columns[1]])
            ...
            dblock = DataBlock(
                ...
                splitter=IndexSplitter(valid_idx),  # Use stratified split here
                item_tfms=[
                    Resize(1024),
                    custom_callback  # Custom transformations
                ],
                batch_tfms=aug_transforms(
                    size=600,
                    min_scale=0.75,
                    max_zoom=1.1,  # Slight zoom to create diversity
                    max_rotate=20.0  # Slight rotation
                )
            )
