...
# einav
    ...
    def train_model(self, dls, epochs=10, save_path='model.pth'):  # CHNGES: Added save path
        """train_model uses Learner's fine_tune method for training and saves the model"""
        if self.class_learner is None:
            self.class_learner = Learner(dls, self, loss_func=CrossEntropyLossFlat(), metrics=accuracy)
        self.class_learner.model.to('cpu')    
        self.class_learner.fine_tune(epochs) # CHNGES: Saving the model after training
        
        # Save the model after training
        self.save_model(save_path)


    def save_model(self, path):  # CHNGES: Added save model method
        """Save model state to a file"""
        self.class_learner.model.to('cpu')  # CHNGES: Move model to CPU for saving
        self.class_learner.model.cpu()
        torch.save(self.class_learner.model.state_dict(), path)

    def load_model(self, path):  # CHNGES: Added load model method
        """Load model state from a file"""
        self.class_learner.model.load_state_dict(torch.load(path, map_location='cpu'))  # CHNGES: Move to CPU on loading
        self.class_learner.model.to('cuda')  # CHNGES: Move back to GPU if needed
        self.class_learner.model.eval()  # CHNGES: Set to evaluation mode
    ...
    def predict(self, img_path):
        ...
        img = PILImage.create(img_path).to('cuda')  # CHNGES: Move image to GPU
        return self.class_learner.predict(img)  # CHNGES: Pass image to predict
