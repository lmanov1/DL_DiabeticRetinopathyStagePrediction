...
##### Class: PretrainedEyeDiseaseClassifier
    ...
    - Methods:
        ...
        - train_model(self, dls, epochs=10): Trains the model using the given DataLoaders.
        - evaluate_model(self, dls): Evaluates the model and prints metrics.

##### Class: EyeDiseaseClassifier    

    This is a generic CNN classification model that can be used for different eye disease diagnostics ( based on retina fundus images).  
    The model can be trained on different datasets, each with disease-related labels (based on ‘num classes’ parameter which defines the last decision layer shape). The class uses Fastai data loaders that allow dataset iteration on (label, image) batches.
    - Methods
...
#### Main flow      
    ...
    - Trains and evaluates both pre-trained and inference models      
    - Use Fastai's Learner class to handle training and evaluation      

#### Util      
    MultiPlatform.py            
        - get_path_separator(): Determines and returns the appropriate path separator based on the operating system.        
        - get_home_directory(): Retrieves and returns the home directory path for the current user based on the operating system.        Implement data augmentation techniques.
